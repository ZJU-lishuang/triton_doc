...
结果表明，我们的非优化模型配置提供了大约每秒 73 次推理的吞吐量。请注意，从一个并发请求到两个并发请求吞吐量显著增加，之后吞吐量趋于平稳。对于一个并发请求，Triton 在响应返回到客户端并且在服务器接收到下一个请求期间处于空闲状态。由于Triton 将一个请求的处理与另一个请求的通信重叠，吞吐量随着单个并发请求增加到两个而增加。因为我们在与 Triton 相同的系统上运行 perf_analyzer，所以两个请求足以完全隐藏通信延迟。
